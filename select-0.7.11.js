 ...
 * -  `xpath(query,scope?,limit?)`
 *
 * Differences with jQuery
 * -----------------------
 *
 * - SVG nodes are supported
 * - XPath queries are supported (`xpath`)
...
//       then pooled, then applied. It could work like this:
//
//       $(".some.selector").virtual()
//       <do stuff with the selection>
//       .apply();
//
//       Likewise, we should add tracking
//
//       $(".some.selector").track({handlers})
//       .untrack({handlers}.
//
 ...
 *       function returns an array of the matching element nodes.
*/
var xpath = function(selector, scope) {
	if (typeof(scope) == "string" || Selection.IsSelection(scope)) {
		// If the input is a string, we convert it to a selection
		var r = new Selection();
		var s = new Selection(scope);
		for (var i=0 ; i<s.length ; i++){
			r.expand(xpath(selector, s[i]));
		}
		return r;
	} else {
		scope              = scope||document.firstElementChild;
		// http://stackoverflow.com/questions/11022718/using-creatensresolver-with-multiple-namespaces-specified-in-the-xml
		// TODO: The query will only resolve the elements within the same namespace
		// unless the resolver is set.
		var scope_document = scope.ownerDocument == null ? document : scope.ownerDocument;
		var resolver       = document.createNSResolver( scope_document );
		var result         = scope_document.evaluate(selector, scope, resolver, XPathResult.ANY_TYPE, null );
		// SEE: https://www.nczonline.net/blog/2009/03/17/xpath-in-javascript-part-1/
		var t = result.resultType;
		var r = new Selection();
		...
		} else if (t == XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE || t == XPathResult.ORDERED_NODE_SNAPSHOT_TYPE) {
			for (var i=0 ; i <result.snapshotLength ; i++) {
				r.expand (result.snapshotItem(i));
			}
		} else if (t == XPathResult.ANY_UNORDERED_NODE_TYPE || t == XPathResult.FIRST_ORDERED_NODE_TYPE) {
			r.expand(result.singleNodeValue);
		} else {
			console.error("select.xpath: Unsupported XPathResult type", result.resultType)
		}
		return r
	}
}
...
/**
 * @constructor
 * Fix: Added to remove closure-compiler's error when `this` is referenced.
*/
...
Selection.AsElementList = function(value) {
	...
	if (value.correspondingUseElement) {
		/**
			We check if the value is an 'SVGElementInstance' by testing for the
			'correspondingUseElement' property, which will correspond to the
			DOM Node of interest.
			For more details see http://html5index.org/SVG%20-%20SVGElementInstance.html
		*/
		value = value.correspondingUseElement;
	};
...
/**
 * `Selection.XPath(query:String, scope:String|Node|Selection|None)`
 *
 * :	Returns the result of executing the given XPath query on thee
 * 		selection.
*/
Selection.XPath = function (query, scope) {
	return xpath(query, scope);
}
...
/**
 * `Selection.xpath(query)`
 *
 * :	Returns the results of running the given XPath query on all the
 *      nodes within the selection.
 *
 *      - `query` the XPath query, as a string
 *
*/
Selection.prototype.xpath  = function( query ) {
	return Selection.XPath(query, this);
}
 ...
 * `Selection.index()`
 *
 * :	Returns the number of previous siblings to this node.
*/
Selection.prototype.index = function( selector ) {
	var node = this[0];
	if (typeof (selector) == "string") {
		selector = function(v){$(v).like(selector)};
	} else if (typeof (selector) == "function") {
		selector = selector;
	} else if ( selector ) {
		console.error("Selection.index: Unsupported selector, expected string or function, got:" ,selector)
		return -1;
	}
	if (node) {
		...
		var index    = 0;
		while (previous) {
			if      (selector) { index += selector(previous) ? 1 : 0; }
			else if (Selection.IsElement(previous)) {index += 1;}
...
// FIXME: This might need to be rewritten
/**
...
Selection.prototype.after = function( value ) {
	...
	while (scope && !Selection.IsElement(scope)) {scope = scope.nextSibling;}
	if (scope) {
		// If we have a scope, this means we invoke insertBefore
		if (Selection.Is(value)) {
			for (var i=0 ; i<value.length ; i++) {
				// FIXME: Might need to recurse there
				scope.parentNode.insertBefore(value[i], scope);
			}
		} else if (typeof value == "string") {
			scope.parentNode.insertBefore(scope.ownerDocument.createTextNode(value), scope);
...
Selection.prototype.before = function( value ) {
	...
	if (Selection.Is(value)) {
		...
		parent.insertBefore(scope.ownerDocument.createElement(value[i]), scope);
	} else if (typeof value.length != "undefined") {
		for (var i=0 ; i<value.length ; i++) {
			parent.insertBefore(value[i], scope);
		}
...
Selection.prototype.contains = function(node) {
	if (node instanceof Array || Selection.IsSelection(node)) {
		var found = false;
	...
	if (!name) {
		...
		if (typeof value == "undefined") {
			for (var i=0 ; i < this.length ; i++ ) {
				...
				value = typeof node.dataset != "undefined" ? node.dataset[name] : attr_value;
...
Selection.prototype.addClass = function( className ) {
	...
	for (var i=0 ; i < this.length ; i++ ) {
		...
		if (node.classList) {
			...
			// Otherwise we emulate it
			var c   = node.getAttribute("class");
			if (c && c.length > 0) {
				var s = c.split(" ");
				var found = false;
				for (var j=0 ; !found && j<s.length ; j++) {
					found = s[j] == className;
				}
				if (!found) {
					node.setAttribute("class", c + " " + className);
				}
			} else {
...
Selection.prototype.removeClass = function( className ) {
	for (var i=0 ; i < this.length ; i++ ) {
		...
		if (node.classList) {
			...
			// FIXME: This is slow, but it works on IE unlike the code
			// commented out below
			node.setAttribute(
				"class",
				(node.getAttribute("class")||"").split(" ").filter(function(_){return _!=className}).join(" ")
			);
		}
	}
	return this;
...
*/
Selection.prototype.focus = function(callback) {
...
// -- MODULE EXPORT -----------------------------------------------------------
// NOTE: IE11 does not have Object.assign
Object.getOwnPropertyNames(select).forEach(function(k){exports[k]=select[k];});
if (typeof window.extend !== "undefined") {window.extend.module("select", select)}
if (typeof window !== "undefined") {
	window.select = select;
	if (!window.S) { window.S = select.$ }
